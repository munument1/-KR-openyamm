...
def main() -> int:
    ...
    if translation_ids != expected_ids:
        raise ValueError(
            f"Arcomage translation ID mismatch: missing={sorted(expected_ids - translation_ids)}, "
            f"extra={sorted(translation_ids - expected_ids)}"
        )
    ...
    if set(source_rows) != expected_ids:
        raise ValueError(
            f"Arcomage source ID mismatch: missing={sorted(expected_ids - set(source_rows))}, "
            f"extra={sorted(set(source_rows) - expected_ids)}"
        )
    ...
    for card_id in range(102):
        ...
        if spec.get("source") != source_text:
            raise ValueError(
                f"Arcomage source drift at ID {card_id}: expected {spec.get('source')!r}, got {source_text!r}"
            )
