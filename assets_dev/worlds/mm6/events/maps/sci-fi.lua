...
SetMapMetadata({
    ...
    [29] = { kind = "open_chest", source = "opcode", chestIds = {0} },
    [30] = { kind = "open_chest", source = "opcode", chestIds = {1} },
    [31] = { kind = "open_chest", source = "opcode", chestIds = {2} },
    [32] = { kind = "open_chest", source = "opcode", chestIds = {3} },
    [33] = { kind = "open_chest", source = "opcode", chestIds = {4} },
    [34] = { kind = "open_chest", source = "opcode", chestIds = {5} },
    [35] = { kind = "open_chest", source = "opcode", chestIds = {6} },
    [36] = { kind = "open_chest", source = "opcode", chestIds = {7} },
    [37] = { kind = "open_chest", source = "opcode", chestIds = {8} },
    [47] = { kind = "secret_event", source = "heuristic", hidden = true },
    [53] = { kind = "generic_event", source = "opcode" },
    [54] = { kind = "generic_event", source = "opcode" },
    [55] = { kind = "generic_event", source = "opcode" },
    [56] = { kind = "generic_event", source = "opcode" },
    [57] = { kind = "generic_event", source = "opcode" },
    [58] = { kind = "generic_event", source = "opcode" },
    [59] = { kind = "generic_event", source = "opcode" },
    [60] = { kind = "generic_event", source = "opcode" },
    [61] = { kind = "generic_event", source = "opcode" },
    [62] = { kind = "secret_event", source = "heuristic", hidden = true },
    [63] = { kind = "secret_event", source = "heuristic", hidden = true },
    [64] = { kind = "secret_event", source = "heuristic", hidden = true },
    [65] = { kind = "open_chest", source = "opcode", chestIds = {9} },
    [66] = { kind = "open_chest", source = "opcode", chestIds = {10} },
    [67] = { kind = "open_chest", source = "opcode", chestIds = {11} },
    [68] = { kind = "open_chest", source = "opcode", chestIds = {12} },
    [69] = { kind = "open_chest", source = "opcode", chestIds = {13} },
    [70] = { kind = "passage", source = "opcode", targetMap = "oracle.blv", targetName = "Oracle of Enroth" },
    },
    textureNames = {"trekscon"},
    spriteNames = {},
    castSpellIds = {15},
    timers = {
    { eventId = 71, sourceEventId = 71, triggerStep = 0, origin = "legacy", triggerKind = "timer", scheduleKind = "interval", intervalHalfMinutes = 3 },
    },
})
...
RegisterEvent(53, "Computer Terminal", function(continueStep)
    if continueStep == 3 then
    end
    if continueStep ~= nil then return end
    evt.SetTexture(2899, "trekscon")
    evt.SetMessage("\"Hello and welcome to this self-guided tour of the Varn Planetary Control Facility.  We gladly welcome all visitors.  As you arrive at each key area, be sure to check any of our display screens for more information.  Enjoy your tour!\"")
    evt._PressAnyKey(53, 3)
end, "Computer Terminal")

RegisterEvent(54, "Computer Terminal", function(continueStep)
    if continueStep == 3 then
    end
    if continueStep ~= nil then return end
    evt.SetTexture(2903, "trekscon")
    evt.SetMessage("\"This is the main equipment storage and repair facility.  By now, you may have noticed several floating Drone-bots.  They are responsible both for maintaining the key systems of this facility as well as sanitation.  If a unit becomes damaged, it is brought here to be repaired.\"")
    evt._PressAnyKey(54, 3)
end, "Computer Terminal")

RegisterEvent(55, "Computer Terminal", function(continueStep)
    if continueStep == 3 then
    end
    if continueStep ~= nil then return end
    evt.SetTexture(2930, "trekscon")
    evt.SetMessage("\"The room to your left is the main meeting hall.  Visiting dignitaries from around the world have feasted at banquets held in their honor.  In fact, it is said that at his 21st birthday party King Sheridan nearly choked to death on a piece of mogred, but was saved by a serving girl who he later married and made his Queen.  Ahh, l’amour.\"")
    evt._PressAnyKey(55, 3)
end, "Computer Terminal")

RegisterEvent(56, "Computer Terminal", function(continueStep)
    if continueStep == 3 then
    end
    if continueStep ~= nil then return end
    ...
    evt._PressAnyKey(56, 3)
end, "Computer Terminal")

RegisterEvent(57, "Computer Terminal", function(continueStep)
    if continueStep == 3 then
    end
    if continueStep ~= nil then return end
    evt.SetTexture(2911, "trekscon")
    evt.SetMessage("\"We apologize for any inconvenience, but we ask that all personnel evacuate the facility at this time.  Please do not be alarmed.  Thank you.\"")
    evt._PressAnyKey(57, 3)
end, "Computer Terminal")

RegisterEvent(58, "Computer Terminal", function(continueStep)
    if continueStep == 3 then
    end
    if continueStep ~= nil then return end
    evt.SetTexture(2914, "trekscon")
    evt.SetMessage("\"Before entering the Planetary Reaction Chamber, please request a pair of Safety Goggles from one of our Drones.  During peak hours of operation, this facility can generate enough power to produce a light bright enough to be seen from space.  Please avoid looking directly into the light.\"")
    evt._PressAnyKey(58, 3)
end, "Computer Terminal")

RegisterEvent(59, "Computer Terminal", function(continueStep)
    if continueStep == 3 then
    end
    if continueStep ~= nil then return end
    evt.SetTexture(2921, "trekscon")
    evt.SetMessage("\"Alert, environmental controls are offline on Level Four, Sections 18 through 96.  Access restricted to drones until further notice.\"")
    evt._PressAnyKey(59, 3)
end, "Computer Terminal")

RegisterEvent(60, "Computer Terminal", function(continueStep)
    if continueStep == 3 then
    end
    if continueStep ~= nil then return end
    evt.SetTexture(2933, "trekscon")
    evt.SetMessage("\"Warning, intruder alert, Level Four, Section.  All drones proceed to Level Four to intercept intruders.  Reactor is offline.  Encryption integrity at 2.064%.  Warning, intruder alert, Level Four…\"")
    evt._PressAnyKey(60, 3)
end, "Computer Terminal")

RegisterEvent(61, "Computer Terminal", function(continueStep)
    if continueStep == 4 then
    end
    if continueStep ~= nil then return end
    evt.SetTexture(2927, "trekscon")
    SetQBit(QBit(1300)) -- NPC
    evt.SetMessage("\"Blaster weapons provide an effective, accurate ranged attack.  To operate the blaster, hold the grip comfortably in your hand, point the barrel at your target, and gently squeeze the trigger.  Should the weapon misfire, do not look into the barrel- give the weapon to an instructor and let them fix the problem.  Never point a blaster at something you do not want to vaporize.\"")
    evt._PressAnyKey(61, 4)
end, "Computer Terminal")
...
RegisterEvent(70, "Exit", function()
    evt.MoveToMap(-1433, 2204, -495, 1536, 0, 0, 0, 0, "oracle.blv") -- Oracle of Enroth
end, "Exit")
