...
SetMapMetadata({
    ...
    [451] = { kind = "passage", source = "opcode", targetMap = "d34.blv", targetName = "Small Sub Pen" },
...
RegisterEvent(6, nil, function()
    ...
    if evt.CheckMonstersKilled(ActorKillCheck.UniqueNameId, 1, 0, true) then -- unique actor 1 "Blackwell Cooper"; all matching actors defeated
        ...
        SetQBit(QBit(6)) -- Pirate Leader in Dagger Wound Pirate Outpost killed (quest given at Q Bit 5). Ends pirate/lizardman war on Dagger Wound. Shuts off pirate timer.
    end
end)
...
RegisterEvent(61, "Key Hole", function()
    evt.ForPlayer(Players.All)
    if not HasItem(619) then -- Pirate Leader's Key
        evt.StatusText("You don't have the Key")
        return
    end
...
RegisterEvent(451, "Enter the Submarine", function()
    ...
    if HasItem(619) then -- Pirate Leader's Key
        ...
        evt.MoveToMap(-2416, 1850, -687, 1536, 0, 0, 0, 0, "d34.blv") -- Small Sub Pen
        return
    end
...
RegisterEvent(501, "Leave the Regnan Pirate Outpost", function()
    evt.MoveToMap(4217, -17375, 146, 1536, 0, 0, 0, 1, "out01.odm") -- Dagger Wound Island
end, "Leave the Regnan Pirate Outpost")
