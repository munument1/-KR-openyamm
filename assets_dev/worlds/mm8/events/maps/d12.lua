...
SetMapMetadata({
    ...
    [101] = { kind = "secret_event", source = "heuristic", hidden = true },
    [102] = { kind = "secret_event", source = "heuristic", hidden = true },
    [103] = { kind = "secret_event", source = "heuristic", hidden = true },
    [104] = { kind = "secret_event", source = "heuristic", hidden = true },
    [105] = { kind = "secret_event", source = "heuristic", hidden = true },
    [106] = { kind = "secret_event", source = "heuristic", hidden = true },
    [107] = { kind = "secret_event", source = "heuristic", hidden = true },
    [108] = { kind = "secret_event", source = "heuristic", hidden = true },
    [501] = { kind = "leave_dungeon", source = "opcode", targetMap = "out03.odm", targetName = "Alvar" },
    },
    textureNames = {},
    spriteNames = {},
    castSpellIds = {15},
...
RegisterEvent(6, nil, function()
    ...
    if not evt.CheckMonstersKilled(ActorKillCheck.MonsterId, 105, 0, false) then return end -- monster 105 "Ogre Mage Apprentice"; all matching actors defeated
    if not evt.CheckMonstersKilled(ActorKillCheck.MonsterId, 106, 0, false) then return end -- monster 106 "Ogre Mage "; all matching actors defeated
    if not evt.CheckMonstersKilled(ActorKillCheck.MonsterId, 107, 0, false) then return end -- monster 107 "Ogre Magi"; all matching actors defeated
    if not IsQBitSet(QBit(131)) then -- Ogre Questbit set for Riki
        SetQBit(QBit(131)) -- Ogre Questbit set for Riki
        evt.SummonMonsters(1, 1, 160, 32, 0, 0, 1, 0) -- encounter slot 1 "Ogre Warrior" tier A, count 160, pos=(32, 0, 0), actor group 1, no unique actor name
...
RegisterEvent(12, "Key Hole", function()
    ...
    if evt.CheckMonstersKilled(ActorKillCheck.Group, 5, 1, false) then return end -- actor group 5: Greater Wererat; at least 1 matching actor defeated
    if IsQBitSet(QBit(120)) then return end -- Rescued Smuggler Leader's Familly
    SetQBit(QBit(120)) -- Rescued Smuggler Leader's Familly
    evt.SpeakNPC(102) -- Irabelle Hunter
end, "Key Hole")
