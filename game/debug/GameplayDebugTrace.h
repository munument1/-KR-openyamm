...
#define GAMEPLAY_PROJECTILE_TRACE(message) \
    do \
    { \
        if (::OpenYAMM::Game::gameplayProjectileTraceEnabled()) \
        { \
            ::OpenYAMM::Game::gameplayProjectileTraceWrite(message); \
        } \
    } while (false)

#define GAMEPLAY_PROJECTILE_TRACE_BLOCK(...) \
    do \
    { \
        if (::OpenYAMM::Game::gameplayProjectileTraceEnabled()) \
        { \
